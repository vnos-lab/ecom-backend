...
func NewSearchClient(config config.Config, logger *zap.Logger) *Client {
	c := &Client{}
	logger.Info("Connecting to redis-search...")
	c.InitializeConnection(config.Redis.Host, config.Redis.Name, config.Redis.Password)
	return c
}
...
func (c *Client) InitializeConnection(addr, name, password string) {
	...
	// Create a schema
	_ = redisearch.NewSchema(redisearch.DefaultOptions).
